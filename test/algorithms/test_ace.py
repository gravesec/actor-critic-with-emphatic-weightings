...
class ACETests(unittest.TestCase):
    ...
    def test_binary_ace_on_policy(self):
        ...
        ax.errorbar(np.arange(num_episodes), mean_rewards, yerr=[ci, ci], label='TD Error AC')
    ...
    def test_aa_binary_ace_on_policy(self):
        ...
        fig = plt.figure()
        ax = fig.add_subplot(111)
        ax.errorbar(np.arange(num_episodes), mean_rewards, yerr=[ci, ci], label='All Actions AC')
        plt.legend(loc='lower right')
        plt.title('Mountain Car (on-policy)')
        plt.xlabel('Episode Number')
        plt.ylabel('Total Reward')
