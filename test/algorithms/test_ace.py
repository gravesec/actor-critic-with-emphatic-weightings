...
class ACETests(unittest.TestCase):
    ...
    def test_binary_ace_on_policy(self):
        ...
        actor_step_size = .05
        critic_step_size = .2
        critic_step_size2 = .01
        critic_trace_decay_rate = .9
        ...
        critic = BinaryTDC(tc.total_num_tiles, critic_step_size / tc.num_active_features, critic_step_size2 / tc.num_active_features, critic_trace_decay_rate)
        ...
        for episode_num in tqdm(range(num_episodes)):
            ...
            for t in range(1000):
                ...
                actor.learn(0., 1., actor_step_size / tc.num_active_features, 1., delta_t, indices_t, a_t, f_t)  # Update actor.
    ...
    def test_aa_binary_ace_on_policy(self):
        ...
        tc = TileCoder(np.array([env.observation_space.low, env.observation_space.high]).T, [5, 5], 8, True)
        actor = BinaryACE(env.action_space.n, tc.total_num_tiles)
        critic = BinaryGQ(env.action_space.n, tc.total_num_tiles, critic_step_size / tc.num_active_features, critic_step_size2 / tc.num_active_features, critic_trace_decay_rate)
        ...
        for episode_num in tqdm(range(num_episodes)):
            ...
            for t in range(1000):
                ...
                actor.all_actions_learn(critic.estimate(indices_t), indices_t, 0., 1., actor_step_size / tc.num_active_features, f_t)
                indices_t = indices_tp1
                f_t *= gamma
                g += r_tp1
                if terminal:
    ...
    def test_binary_ace_off_policy(self):
        ...
        for run_num in range(num_runs):
            ...
            for t in tqdm(range(num_timesteps)):
                ...
                actor.learn(i_t, eta, alpha_a, rho_t, delta_t, indices_t, a_t, f_t)
                gamma_t = gamma_tp1
                indices_t = indices_tp1
                rho_tm1 = rho_t
