...
from src.algorithms.low_var_ace import LinearLowVarACE, BinaryLowVarACE
...
class ACETests(unittest.TestCase):
    ...
    def test_learn_one(self):
        ace = BinaryLowVarACE(2, 3)
        indices_t = np.array([2], dtype=np.intp)
        a_t = 0
        delta_t = 1  # action was better than expected
        before = ace.pi(indices_t)
        ace.learn(gamma_t=.9, i_t=1, eta_t=1., alpha_t=.1, rho_t=1., delta_t=delta_t, indices_t=indices_t, a_t=a_t, F_t=.5)
    ...
    def test_binary_ace_on_policy(self):
        ...
        actor = BinaryLowVarACE(env.action_space.n, tc.total_num_tiles)
        ...
        for t in tqdm(range(num_timesteps)):
            ...
            actor.learn(gamma_t, i_t=1., eta_t=1 if gamma_t == 0. else 0., alpha_t=.001, rho_t=1., delta_t=delta_t, indices_t=indices_t, a_t=a_t, F_t=F_t)  # Update actor.
    ...
    def test_binary_ace_off_policy(self):
        ...
        for run_num in range(num_runs):
            ...
            for t in tqdm(range(num_timesteps)):
                ...
                actor.learn(gamma_t, 1., eta, alpha_a, rho_t, delta_t, indices_t, a_t, F_t=F_t)
